...
Clock.Frame, Clock.Stroke = CreateHUDFrame("Clock", UDim2.new(1, -8, 0, 8), UDim2.new(0, 0, 0, HUD_HEIGHT))
...
function Clock.Update()
	...
	if Clock.Type == 24 then
		Clock.Text.Text = string.format("%02d:%02d:%02d", now.hour, now.min, now.sec)
	else
		local hour = now.hour
		local suffix = "AM"
		...
		Clock.Text.Text = string.format("%02d:%02d:%02d %s", hour, now.min, now.sec, suffix)
	end
end
...
Coords.Frame, Coords.Stroke = CreateHUDFrame("Coords", UDim2.new(0, 8, 0, 8), UDim2.new(0, 0, 0, HUD_HEIGHT))
...
function Coords.Update()
	...
	Coords.Text.Text = string.format("XYZ: %.2f, %.2f, %.2f", position.X, position.Y, position.Z)
end
...
FPS.Frame, FPS.Stroke = CreateHUDFrame("FPS", UDim2.new(1, -8, 0, 38), UDim2.new(0, 0, 0, HUD_HEIGHT))
...
function FPS.Update(deltaTime)
	...
	if FPS.Frame.Visible then
		FPS.Text.Text = string.format("FPS: %.0f", FPS.Value)
	end
end
...
Keystrokes.Frame, Keystrokes.Stroke = CreateHUDFrame("Keystrokes", UDim2.new(0, 8, 1, -8), UDim2.new(0, 150, 0, 190))
...
Keystrokes.Keys = {
	W = CreateKey(Keystrokes.Frame, "W", CENTER_X, 8, KEY_WIDTH, KEY_HEIGHT),

	A = CreateKey(Keystrokes.Frame, "A", LEFT_X, 8 + KEY_HEIGHT + GAP, KEY_WIDTH, KEY_HEIGHT),

	S = CreateKey(Keystrokes.Frame, "S", CENTER_X, 8 + KEY_HEIGHT + GAP, KEY_WIDTH, KEY_HEIGHT),

	D = CreateKey(Keystrokes.Frame, "D", RIGHT_X, 8 + KEY_HEIGHT + GAP, KEY_WIDTH, KEY_HEIGHT),

	LMB = CreateKey(Keystrokes.Frame, "LMB", LEFT_X, 8 + (KEY_HEIGHT + GAP) * 2, KEY_WIDTH, 30),

	RMB = CreateKey(Keystrokes.Frame, "RMB", CENTER_X, 8 + (KEY_HEIGHT + GAP) * 2, KEY_WIDTH, 30),

	CPS = CreateKey(Keystrokes.Frame, "CPS", RIGHT_X, 8 + (KEY_HEIGHT + GAP) * 2, KEY_WIDTH, 30),
}

Keystrokes.Space =
	CreateKey(Keystrokes.Frame, "Space", LEFT_X, 8 + (KEY_HEIGHT + GAP) * 2 + 30 + GAP, KEY_WIDTH * 3 + GAP * 2, 30)

--FIXME: uncomment the below lines
...
local function SetKeyActive(key, active)
	key.BackgroundColor3 = active and ACTIVE_KEY_COLOR or INACTIVE_KEY_COLOR
end
...
function Keystrokes.Update()
	...
	SetKeyActive(Keystrokes.Keys.W, UserInputService:IsKeyDown(keyCodes.W))

	SetKeyActive(Keystrokes.Keys.A, UserInputService:IsKeyDown(keyCodes.A))

	SetKeyActive(Keystrokes.Keys.S, UserInputService:IsKeyDown(keyCodes.S))

	SetKeyActive(Keystrokes.Keys.D, UserInputService:IsKeyDown(keyCodes.D))

	SetKeyActive(Keystrokes.Keys.LMB, UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1))

	SetKeyActive(Keystrokes.Keys.RMB, UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2))

	SetKeyActive(Keystrokes.Space, UserInputService:IsKeyDown(Enum.KeyCode.Space))
...
function Legits.StartEditMode()
	...
	for _, module in ipairs(GetVisibleModules()) do
		...
		if frame.Visible then
			...
			table.insert(
				EditConnections,
				module._editOverlay.InputBegan:Connect(function(input)
