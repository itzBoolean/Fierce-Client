...
local Vector2new, Vector3zero, CFramenew, Color3fromRGB, Color3fromHSV, Drawingnew, TweenInfonew =
	Vector2.new, Vector3.zero, CFrame.new, Color3.fromRGB, Color3.fromHSV, Drawing and Drawing.new, TweenInfo.new
local mousemoverel, mousemoveabs, tablefind, tableremove, stringlower, stringsub, mathclamp =
...
local clonefunction, cloneref =
	clonefunction or LPH_NO_VIRTUALIZE(function(...)
		return ...
...
local RequiredDistance, Typing, Running, RightClickHeld, ServiceConnections, Animation, OriginalSensitivity = 2000, false, false, false, {}
...
getgenv().ExunysDeveloperAimbot = {
	...
	Settings = {
		...
		Toggle = false,
		RequireRightClick = false, -- When true, right mouse button must be held while aiming.
	},
...
local GetClosestPlayer = LPH_NO_VIRTUALIZE(function(Aux)
	...
	if not Environment.Locked or Aux then
		...
		for _, Value in next, GetPlayers(Players) do
			...
			if
				...
				and Humanoid
			then
				local PartPosition, TeamCheckOption =
					__index(Character[LockPart], "Position"), Environment.DeveloperSettings.TeamCheckOption
	...
	elseif
		(GetMouseLocation(UserInputService) - ConvertVector(
			WorldToViewportPoint(
				Camera,
				__index(__index(__index(Environment.Locked, "Character"), LockPart), "Position")
			)
...
local Load = function()
	...
	local Settings, Triggerbot, Tracer, FOVCircle, FOVCircleOutline, TracerSettings, FOVSettings, Offset =
		Environment.Settings,
		Environment.Triggerbot,
		Environment.Tracer,
		Environment.FOVCircle,
		Environment.FOVCircleOutline,
		Environment.ClosestPlayerTracer,
	...
	if mouse1press and mouse1release then
		ServiceConnections.Triggerbot = Connect(
			__index(RunService, UpdateMode),
			LPH_NO_VIRTUALIZE(function()
				if Triggerbot.Enabled and Mouse.Target then
					...
					if Character and Humanoid and Player then
						if
							Triggerbot.TeamCheck
							and __index(Player, TeamCheckOption) == __index(LocalPlayer, TeamCheckOption)
						then
							return
						end
	...
	ServiceConnections.UpdateTracer = Connect(
		__index(RunService, UpdateMode),
		LPH_NO_VIRTUALIZE(function()
			...
			if ClosestPlayer then
				...
				setrenderproperty(
					Tracer,
					"Color",
					TracerSettings.RainbowColor and GetRainbowColor() or TracerSettings.Color
				)
				...
				setrenderproperty(
					...
					ConvertVector(
						WorldToViewportPoint(
							Camera,
							__index(__index(ClosestPlayer, "Character")[Settings.LockPart], "Position")
						)
					)
				)
			else
		...
		LPH_NO_VIRTUALIZE(function()
			if FOVSettings.Enabled and Settings.Enabled then
				...
				setrenderproperty(
					FOVCircle,
					"Color",
					(Environment.Locked and FOVSettings.LockedColor)
				...
				setrenderproperty(
					FOVCircleOutline,
					"Color",
					FOVSettings.RainbowOutlineColor and GetRainbowColor() or FOVSettings.OutlineColor
				)
			...
			local ShouldAim = Settings.Enabled and (not Settings.RequireRightClick or RightClickHeld)

			if not ShouldAim and Environment.Locked then
				CancelLock()
			end
			...
			if ShouldAim then
				...
				Offset = OffsetToMoveDirection
						and __index(
							FindFirstChildOfClass(__index(Environment.Locked, "Character"), "Humanoid"),
							"MoveDirection"
				...
				if Environment.Locked then
					...
					if Settings.LockMode == 2 then
						mousemoverel(
							(LockedPosition.X - GetMouseLocation(UserInputService).X) / Settings.Sensitivity2,
							(LockedPosition.Y - GetMouseLocation(UserInputService).Y) / Settings.Sensitivity2
						)
	...
	ServiceConnections.InputBeganConnection = Connect(
		__index(UserInputService, "InputBegan"),
		LPH_NO_VIRTUALIZE(function(Input)
			...
			if
				Input.UserInputType == Enum.UserInputType.Keyboard and Input.KeyCode == TriggerKey
				or Input.UserInputType == TriggerKey
			then
				if Toggle then
					Running = not Running
	...
	ServiceConnections.InputEndedConnection = Connect(
		__index(UserInputService, "InputEnded"),
		LPH_NO_VIRTUALIZE(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton2 then
				RightClickHeld = false

				if Settings.RequireRightClick then
					CancelLock()
				end
...
Environment.Blacklist = LPH_NO_VIRTUALIZE(
	function(self, Username) -- ExunysDeveloperAimbot.Blacklisted(self, <string> Player Name)
...
Environment.Whitelist = LPH_NO_VIRTUALIZE(
	function(self, Username) -- ExunysDeveloperAimbot.Whitelist(self, <string> Player Name)
